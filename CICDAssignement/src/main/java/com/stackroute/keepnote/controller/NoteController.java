...
public class NoteController {
	...
	public ResponseEntity<Note> deleteNote(@PathVariable("id") int id,HttpSession session) {
		String userId = (String) session.getAttribute("loggedInUserId");
		if(userId==null) {
			return new ResponseEntity<Note>(HttpStatus.UNAUTHORIZED);
		}
		boolean bool=false;
		try {
			bool = noteService.deleteNote(id);
	 ...
	 */
	@PutMapping("/note/{id}")
	public ResponseEntity<Note> updateNote(@RequestBody Note note,@PathVariable("id") int id,HttpSession session) {
		String userId = (String) session.getAttribute("loggedInUserId");
		...
		try {
			if(noteService.updateNote(note, id) != null ) {
				return new ResponseEntity<Note>(HttpStatus.OK);
			}
