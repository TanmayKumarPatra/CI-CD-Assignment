...
public class CategoryController {
	...
	public ResponseEntity<Category> createCategory(@RequestBody Category category, HttpSession session) {
		...
		Boolean bool = categoryService.createCategory(category);
		if(bool) {
	...
	public ResponseEntity<Category> deleteCategory(@PathVariable("id") int id, HttpSession session) {
		String userId = (String) session.getAttribute("loggedInUserId");
		if(userId==null) {
			return new ResponseEntity<Category>(HttpStatus.UNAUTHORIZED);
		}
	...
	public ResponseEntity<Category> updateCategory(@RequestBody Category category,@PathVariable("id") int userId,
			HttpSession session) {
		String SessionUser = (String) session.getAttribute("loggedInUserId");
		if(SessionUser==null) {
			return new ResponseEntity<Category>(HttpStatus.UNAUTHORIZED);
		}
		category.setCategoryCreatedBy(SessionUser);
		Category cat = null;
		try {
			cat = categoryService.updateCategory(category, userId);
	...
	public ResponseEntity<List<Category>> categoryById(HttpSession session) {
		...
		List<Category> list = categoryService.getAllCategoryByUserId(SessionUser);
